...
fn default_path(t: AppDataType, product_name: &'static str) -> Option<PathBuf> {
	let app_info = AppInfo { name: product_name, author: AUTHOR };
...
pub fn default_local_path() -> String {
	default_path(AppDataType::UserCache, PRODUCT).map(|p| p.to_string_lossy().into_owned()).unwrap_or_else(|| "$HOME/.openethereum".to_owned())
}
